...
export default class CalculationService {
    ...
    cleanBoard(grid) {
        grid.forEach((row, rowIndex) => {
            row.forEach((currentNumber, columnIndex) => {
                this._existsInRow(grid, rowIndex, columnIndex, currentNumber);
                this._existsInColumn(grid, columnIndex, rowIndex, currentNumber);
    ...
    _existsInRow(grid, rowNumber, currentColumn, num) {
        for (let col = 0; col < 9; col++) {
            if (num !== 0 && col !== currentColumn && grid[rowNumber][col] === num) {
                grid[rowNumber][col] = 0;
            }
        }
    }

    _existsInColumn(grid, column, currentRow, num) {
        for (let row = 0; row < 9; row++) {
            if (num !== 0 && row !== currentRow && grid[row][column] === num) {
                grid[row][column] = 0;
            }
        }
    }
